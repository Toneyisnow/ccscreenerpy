...
coin_id_list = ["BTC", "ETH", "EOS", "BCH", "BNB", "XMR", "XRP", "LTC", "TRX"]
coin_prices = {}
...
def execute_buy(balance, day_index):
    ...
    if (balance.get_cash_balance() <= 0 and balance.get_coin_position("USDT") <= 0):
    ...
    even_cash_value = balance.get_cash_balance() / len(candidate_coins)
...
def execute_sell(balance, day_index):
    ...
    transaction =compose_buy_transaction(balance, day_index, "USDT", balance.get_cash_balance())
    if (transaction != None):
        new_transactions.append(transaction)
...
for day_index in range(start_day_index, end_day_index):
    ...
    if (cci30_data["close"][day_index - 2] > ma40[day_index - 2] and cci30_data["close"][day_index - 1] > ma40[day_index - 1] and
        ...
        trans = execute_buy(balance, day_index)
        if trans != None:
    ...
    # 2. T日的综合指数不被T-2日包含
        ...
        (cci30_data["high"][day_index] > cci30_data["high"][day_index - 2] or cci30_data["low"][day_index] < cci30_data["low"][day_index - 2])):
        trans = execute_sell(balance, day_index)
        if trans != None:
            all_transactions += trans
